      ...
      async function handleApprove(item)
      {
        ...
        if (book){
              ...
              await addDoc(collection(db, "Booking History" ), {
                name:item.name,addhar:item.addhar,contact:item.contact,
                plot:item.plot,mode:item.mode,offer:item.offer,
                });
                await deleteDoc(doc(db, "Booking Request", user.email));
                navigate('/admin/bookingRequest')
          }
    ...
    <div className={styles.userTable}>
      <table >
        ...
        <tbody>
              ...
              <td>{item.mode}</td>
              <td>{item.offer}</td>
              <td><button onClick={()=>handleApprove({
                name:item.name,addhar:item.addhar,contact:item.contact,
                plot:item.plot,mode:item.mode,offer:item.offer
              })} style={{border:'none',backgroundColor:"transparent"}}><img className={styles.asideIcon} src="/images/chek.png" alt="" /></button><button onClick={()=>handleDelete(item.project,item.plot)} style={{border:'none',backgroundColor:"transparent"}}><img className={styles.asideIcon} src="/images/delete.png" alt="" /></button></td>
              </tr>)}
        </tbody>
      </table>
